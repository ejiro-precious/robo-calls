...
if (isset($_POST) && $_POST['direction'] == "Outbound") {
    if (isset($_POST['dtmfDigits']) && $_POST['isActive'] == "1") {
        ...
        if ($digit == '1') {
            $content = "https://s3.eu-west-2.amazonaws.com/peakramadan/peak_audio/english/adhkar_prayers/prayer_menu.mp3";
        } else if ($digit == '2') {
            $content = "https://s3.eu-west-2.amazonaws.com/peakramadan/peak_audio/english/99names.mp3";
        } else if ($digit == '3') {
            $content = "https://s3.eu-west-2.amazonaws.com/peakramadan/peak_audio/english/food_tips/food_tips_menu.mp3";
        } else if ($digit == '4') {
            $content = "https://s3.eu-west-2.amazonaws.com/peakramadan/peak_audio/english/tips_on_health/health_menu.mp3";
        } else if ($digit == '#') {
        ...
        $query = mysqli_query($db, "UPDATE `outbound` SET digit_selected_2='$digit', content_digit_2='$content' WHERE sessionId='$sessionId' AND caller='$callerNumber'");
        ...
        if (mysqli_num_rows($data_x_query) > 0) {
            ...
            $query = mysqli_query($db, "UPDATE `outbound` SET status='$status', callSessionState='$CSS', callerCountryCode='$CCC', currencyCode='$currencyCode', amount='$amount', durationInSecs='$DIS', call_time='$callStartTime', unique_caller='1' WHERE sessionId='$sessionId' AND caller='$callerNumber'");
        }
        if ($query) {
            if ($callerNumber) {
